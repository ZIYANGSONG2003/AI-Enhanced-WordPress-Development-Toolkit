...
// 注册短代码
add_shortcode('user_image_upload', 'syb_user_image_upload_shortcode');

// 显示上传表单和处理上传的函数
function syb_user_image_upload_shortcode() {
    // 检查表单是否提交
    if (isset($_POST['syb_image_upload_submit'])) {
        // 检查上传文件是否存在并且没有错误
        if (isset($_FILES['syb_image_upload']) && $_FILES['syb_image_upload']['error'] === UPLOAD_ERR_OK) {
            // 获取上传文件信息
            $file = $_FILES['syb_image_upload'];
            
            // 检查文件类型是否为图像
            $file_type = wp_check_filetype($file['name']);
            $allowed_types = array('jpg', 'jpeg', 'png', 'gif');
            if (in_array($file_type['ext'], $allowed_types)) {
                // 将文件移动到 WordPress 上传目录
                ...
                if (isset($upload['file'])) {
                    ...
                    echo '<p>Image uploaded successfully: <a href="' . esc_url($upload['url']) . '" target="_blank">View Image</a></p>';
                } else {
    ...
    ob_start();
    ?>
    <form id="syb_image_upload_form" method="post" enctype="multipart/form-data">
        <label for="syb_image_upload">Upload your image:</label>
        <input type="file" name="syb_image_upload" id="syb_image_upload" accept="image/*" required>
        <input type="submit" name="syb_image_upload_submit" value="Upload Image">
    </form>
    <?php
    return ob_get_clean();
}
...
// 注册短代码
add_shortcode('gpt_form', 'gpt_form_shortcode');

// 显示表单的短代码函数
function gpt_form_shortcode() {
    // 输出表单
    ob_start();
    ?>
    <form id="gpt_form" method="post">
        ...
        <input type="submit" name="submit_gpt_form" value="Submit">
    </form>
    <div id="gpt_response"></div>

    <script type="text/javascript">
        document.getElementById('gpt_form').addEventListener('submit', function(event) {
            event.preventDefault();
            var message = document.getElementById('user_message').value;
            
            fetch('<?php echo get_site_url(); ?>/wp-json/gpt/v1/submit', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                document.getElementById('gpt_response').innerHTML = '<p>' + data.result + '</p>';
            })
...
// 处理表单提交的回调函数
function gpt_handle_form_submission(WP_REST_Request $request) {
    $message = sanitize_text_field($request->get_param('message'));

    // 将消息发送到您的 Node.js 服务器
    $response = wp_remote_post('http://13.236.176.215:3000/chat/', array(
        'body' => json_encode(array('message' => $message)),
        'headers' => array('Content-Type' => 'application/json')
    ));

    if (is_wp_error($response)) {
        return new WP_Error('gpt_error', 'Error communicating with GPT server', array('status' => 500));
    }
...
add_action('rest_api_init', function() {
    register_rest_route('gpt/v1', '/submit', array(
        'methods' => 'POST',
        'callback' => 'gpt_handle_form_submission',
    ));
});